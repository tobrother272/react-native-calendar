...
class Calendar extends Component {
  ...
  renderWeekHeader(weekDaysNames) {
    const week = [];
    ...
    return (<View style={this.style.week} key={"weekheader"+Math.random()}>{week}</View>);
  }
  render() {
    ...
    let dayFirst=parseInt(days[0].toString().split("T")[0].split(" ")[1]);
    if(dayFirst!=null){
      //console.log(dayFirst)
    }

    const weekDaysNames = weekDayNames(firstDay);
    const weekHeader=this.renderWeekHeader(weekDaysNames);
    const weeks = [];
    while (days.length) {
      weeks.push(this.renderWeek(days.splice(0, 7), weeks.length));
    }
    weeks.unshift(weekHeader);
